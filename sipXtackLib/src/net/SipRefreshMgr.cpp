    ...
    const UtlString& lineId )
{
    ...
    else
    {
        ...
        if ( isDuplicateRegister(fromUrl, sipMsg) )
        {
			...
			{
				OsReadLock readlock(mRegisterListMutexR);
				removeFromRegisterList(mRegisterList.isSameFrom(fromUrl));
			}
			*/
    ...
    SipMessage *request)
{
    ...
    if ( response->isResponse() )
    {
        ...
        else  // failure case 
        {
			/*RL CHECK!*/
			/*
			SipMessage* requestOriginal = 0;
			{
				OsReadLock readlock(mRegisterListMutexR);
                // Find the request which goes with this response
                SipMessage* requestOriginal = mRegisterList.getRequestFor(response);
			}
			*/
            ...
            if ( isExpiresZero(request) )
            {
                ...
                if ( method.compareTo(SIP_REGISTER_METHOD) == 0 )
                {
					...
					/*
					if (requestOriginal)
						removeFromRegisterList(requestOriginal);
