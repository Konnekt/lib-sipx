...
static SIPX_LINE_DATA* createLineData(SIPX_INSTANCE_DATA* pInst, const Url& uri)
{
    ...
    if (pData != NULL)
    {
		...
		pData->failedUnregister = false; //* RL
...
/*		if (numAddresses > 0) {
		...
		/* RL - wybieramy tylko jakieœ w miarê wiarygodne... */
		for (int i = 0; i < numAddresses; ++i) {
			if (strncmp(addresses[i]->mAddress.data(), "0.", 2) != 0) {
				strcpy(szBindToAddr2, (char*)addresses[i]->mAddress.data());
				break;
			}
		}
        ...
        for (int i = 0; i < numAddresses; i++)
           delete addresses[i];
		if (numAddresses == 0) {
			return SIPX_RESULT_INVALID_STATE;
		}
    }
    else
    {
    ...
    if (bindTo != INADDR_NONE)
    {
        ...
        pInst->pSipUserAgent = new SipUserAgent(
                ...
                NULL,                       // publicAddress
                NULL,                       // defaultUser
                szBindToAddr2,              // default IP Address
...
SIPXTAPI_API SIPX_RESULT sipxLineRemove(SIPX_LINE hLine)
{
    ...
    if (hLine)
    {
        ...
        if (pData)
        {
			...
			bool failedUnregister = pData->failedUnregister;

            sipxLineReleaseLock(pData, SIPX_LOCK_READ) ;
            
            sr = SIPX_RESULT_SUCCESS ;
            if (!bWasRegistered || failedUnregister /*RL*/)   
            {                        
...
/*RL*/
SIPXTAPI_API SIPX_RESULT sipxLineAddCredentialMD5(SIPX_LINE hLine,                                                                  const char* szUserID,
                                                const char* szPasswdMD5,
                                                const char* szRealm) 
{
    SIPX_RESULT sr = SIPX_RESULT_FAILURE ;
    SIPX_LINE_DATA* pData = sipxLineLookup(hLine, SIPX_LOCK_READ) ;
    if (pData)         
	{
        if (szUserID && szPasswdMD5 && szRealm)
		{
...
SIPXTAPI_API SIPX_RESULT sipxBuildMd5UserPasswordDigest(const char* szUserID,
                                               const char* szPasswd,
                                               const char* szRealm,
											   char digest [33]) 
{
	UtlString authToken;
	HttpMessage::buildMd5UserPasswordDigest(szUserID, szRealm, szPasswd, authToken);
	strncpy(digest, authToken.data(), 32);
	digest[32] = 0;
...
SIPXTAPI_API SIPX_RESULT sipxGetStunAddress(const SIPX_INST hInst,
                                              char* address, 
                                              int& port) 
{
    SIPX_RESULT rc = SIPX_RESULT_INVALID_ARGS;
    ...
    if (pInst)
    {
		UtlString addressStr;
		if (! pInst->pSipUserAgent->getNatMappedAddress(&addressStr, &port)) {
			rc = SIPX_RESULT_FAILURE;
		} else {
			rc = SIPX_RESULT_SUCCESS ;
			strcpy(address, addressStr.data());
		}
	}
	return rc;
}
...
SIPXTAPI_API SIPX_RESULT sipxConfigSetPublicAddress(const SIPX_INST hInst,
                                              const char* address, 
                                              int port) 
{
    SIPX_RESULT rc = SIPX_RESULT_INVALID_ARGS;
    ...
    {
		UtlString addressStr;
		pInst->pSipUserAgent->setPublicAddress(address, port);
	}
	return rc;
}
